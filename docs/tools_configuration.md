...
AgentX's tools configuration is located in the `tools` field of `config.json`.
...
By default, AgentX blocks the following dangerous commands:
...
All configuration options can be overridden via environment variables with the format `AGENTX_TOOLS_<SECTION>_<KEY>`:

For example:
- `AGENTX_TOOLS_WEB_BRAVE_ENABLED=true`
- `AGENTX_TOOLS_EXEC_ENABLE_DENY_PATTERNS=false`
- `AGENTX_TOOLS_CRON_EXEC_TIMEOUT_MINUTES=10`

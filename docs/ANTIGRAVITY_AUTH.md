...
**Antigravity** (Google Cloud Code Assist) is a Google-backed AI model provider that offers access to models like Claude Opus 4.6 and Gemini through Google's Cloud infrastructure. This document provides a complete guide on how authentication works, how to fetch models, and how to implement a new provider in AgentX.
...
10. [Creating a New Provider in AgentX](#creating-a-new-provider-in-agentx)
...
**Important:** These credentials are loaded from environment variables:
...
type ProviderAuthResult = {
  ...
  configPatch?: Partial<AgentXConfig>;
  defaultModel?: string;
  notes?: string[];
};
```
...
- Go ≥ 1.21
- AgentX codebase (`pkg/providers/` and `pkg/auth/`)
...
Auth profiles are stored in `~/.agentx/auth.json`:
...
## Creating a New Provider in AgentX

AgentX providers are implemented as Go packages under `pkg/providers/`. To add a new provider:
...
If your provider requires OAuth or special authentication, add a case to `cmd/agentx/cmd_auth.go`:
...
agentx auth login --provider your-provider
...
agentx agent -m "Hello" --model your-model
```
...
export AGENTX_AGENTS_DEFAULTS_MODEL=your-model

# Override provider settings
export AGENTX_MODEL_LIST='[{"model_name":"your-model","model":"your-provider/model-name","api_key":"..."}]'
```
...
### "Token expired"
- Refresh OAuth tokens: `agentx auth login --provider antigravity`
...
- Check auth profile storage: `~/.agentx/auth.json`
- Re-run `agentx auth login --provider antigravity`

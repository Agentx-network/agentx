...
1. `gh workflow run release.yml -f tag=v0.8.36` — creates tag, builds CLI binaries + Docker
2. `build-installers.yml` triggers automatically on release publish — builds all installers
3. Or re-trigger manually: `gh workflow run build-installers.yml -f tag=v0.8.36`

...
// BlockedResult returns a tool error with separate user-facing (ForUser) and
// model-facing (ForLLM) messages. Used when the failure has a natural user
// explanation (vague query, missing consent, 404 with suggestion) so the
// recovery path can show clean text without leaking internal model guidance.

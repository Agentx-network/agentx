...
func (m *Manager) StartAll(ctx context.Context) error {
	...
	for name, channel := range m.channels {
		...
		// Start stream consumer for channels that support streaming
...
func (m *Manager) dispatchStream(ctx context.Context) {
	logger.InfoC("channels", "Stream dispatcher started")

	for {
		select {
		case <-ctx.Done():
			logger.InfoC("channels", "Stream dispatcher stopped")
			return
		default:
			delta, ok := m.bus.SubscribeStream(ctx)
			if !ok {
				continue
			}

			if constants.IsInternalChannel(delta.Channel) {
				continue
			}

			m.mu.RLock()
			channel, exists := m.channels[delta.Channel]
			m.mu.RUnlock()

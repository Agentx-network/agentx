...
func Run(opts Options) (*Result, error) {
	...
	agentxHome, err := resolveAgentXHome(opts.AgentXHome)
	if err != nil {
		return nil, err
	}
	...
	actions, warnings, err := Plan(opts, openclawHome, agentxHome)
	if err != nil {
		return nil, err
	}

	fmt.Println("Migrating from OpenClaw to AgentX")
	fmt.Printf("  Source:      %s\n", openclawHome)
...
func Plan(opts Options, openclawHome, agentxHome string) ([]Action, []string, error) {
	var actions []Action
	var warnings []string
	...
	if !opts.ConfigOnly {
		...
		dstWorkspace := resolveWorkspace(agentxHome)
...
func Execute(actions []Action, openclawHome, agentxHome string) *Result {
	result := &Result{}

	for _, action := range actions {
		switch action.Type {
		case ActionConvertConfig:
			if err := executeConfigMigration(action.Source, action.Destination, agentxHome); err != nil {
...
func executeConfigMigration(srcConfigPath, dstConfigPath, agentxHome string) error {
	...
	if _, err := os.Stat(dstConfigPath); err == nil {
		...
		if err != nil {
			return fmt.Errorf("loading existing AgentX config: %w", err)
		}
...
func resolveAgentXHome(override string) (string, error) {
	if override != "" {
		return expandHome(override), nil
	}
	if envHome := os.Getenv("AGENTX_HOME"); envHome != "" {
		return expandHome(envHome), nil
	}
	home, err := os.UserHomeDir()
	if err != nil {
	...
	return filepath.Join(home, ".agentx"), nil
}

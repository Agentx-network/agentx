...
func TestConvertConfig(t *testing.T) {
	...
	t.Run("unsupported provider warning", func(t *testing.T) {
		...
		if warnings[0] != "Provider 'unknown_provider' not supported in AgentX, skipping" {
	...
	t.Run("unsupported channel warning", func(t *testing.T) {
		...
		if warnings[0] != "Channel 'email' not supported in AgentX, skipping" {
			t.Errorf("unexpected warning: %s", warnings[0])
		}
	})
	...
	t.Run("agent defaults", func(t *testing.T) {
		...
		if cfg.Agents.Defaults.Workspace != "~/.agentx/workspace" {
			t.Errorf("Workspace = %q, want %q", cfg.Agents.Defaults.Workspace, "~/.agentx/workspace")
		}
	})
...
func TestRewriteWorkspacePath(t *testing.T) {
	tests := []struct {
		...
		{"default path", "~/.openclaw/workspace", "~/.agentx/workspace"},
...
func TestRunDryRun(t *testing.T) {
	...
	agentxWs := filepath.Join(agentxHome, "workspace")
	if _, err := os.Stat(filepath.Join(agentxWs, "SOUL.md")); !os.IsNotExist(err) {
		t.Error("dry run should not create files")
	}
	if _, err := os.Stat(filepath.Join(agentxHome, "config.json")); !os.IsNotExist(err) {
...
func TestRunFullMigration(t *testing.T) {
	...
	soulData, err := os.ReadFile(filepath.Join(agentxWs, "SOUL.md"))
	if err != nil {
	...
	agentsData, err := os.ReadFile(filepath.Join(agentxWs, "AGENTS.md"))
	if err != nil {
	...
	memData, err := os.ReadFile(filepath.Join(agentxWs, "memory", "MEMORY.md"))
	if err != nil {
	...
	agentxConfig, err := config.LoadConfig(filepath.Join(agentxHome, "config.json"))
	if err != nil {
		t.Fatalf("loading AgentX config: %v", err)
	}
	if agentxConfig.Providers.Anthropic.APIKey != "sk-ant-migrate-test" {
		t.Errorf("Anthropic.APIKey = %q, want %q", agentxConfig.Providers.Anthropic.APIKey, "sk-ant-migrate-test")
	}
	if agentxConfig.Providers.OpenRouter.APIKey != "sk-or-migrate-test" {
		t.Errorf("OpenRouter.APIKey = %q, want %q", agentxConfig.Providers.OpenRouter.APIKey, "sk-or-migrate-test")
	}
	if !agentxConfig.Channels.Telegram.Enabled {
		t.Error("Telegram should be enabled")
	}
	if agentxConfig.Channels.Telegram.Token != "tg-migrate-test" {
		t.Errorf("Telegram.Token = %q, want %q", agentxConfig.Channels.Telegram.Token, "tg-migrate-test")
	}

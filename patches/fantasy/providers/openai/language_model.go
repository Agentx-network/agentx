...
func (o languageModel) Stream(ctx context.Context, call fantasy.Call) (fantasy.StreamResponse, error) {
	...
	return func(yield func(fantasy.StreamPart) bool) {
				...
				case len(choice.Delta.ToolCalls) > 0:
					...
					for _, toolCallDelta := range choice.Delta.ToolCalls {
						if existingToolCall, ok := toolCalls[toolCallDelta.Index]; ok {
							...
							// When arguments are empty (tool with no parameters),
							// treat as empty JSON object so the tool call is yielded.
							// OpenRouter/Anthropic sends arguments="" for no-param tools
							// instead of "{}".
							if exTc.arguments == "" {
								exTc.arguments = "{}"
		...
		if err == nil || errors.Is(err, io.EOF) {
			...
			// Safety net: yield ToolCall events for any tool calls that were
			// tracked during streaming but never finished (e.g. provider sent
			// tool_calls with empty arguments that the streaming parser missed).
